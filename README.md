...
이 프로젝트는 **Claude Code의 일관된 개발을 위한** 공통 설정 템플릿입니다.

새로운 Java/Spring Boot 프로젝트를 시작할 때 이 템플릿을 기반으로 복시하면, 별도 설정 없이 바로 Claude Code을 활용할 수 있습니다. 
...
- **CLAUDE.md** — Claude가 프로젝트를 이해하고 최적의 도움을 주도록 하는 컨텍스트 문서. 기술 스택, 아키텍처, 개발 패턴 등이 상세히 정리되어 있습니다.
- **setting.json** — Claude Code의 권한(어떤 명령은 허용하고 어떤 명령은 차단할지)과 언어 설정. 보안과 개발 안정성을 함께 고려했습니다.
- **rules/** — 코드 스타일, Git 워크플로우, 커밋 규칙 등 팀이 따를 개발 규칙들. 모든 프로젝트에서 일관되게 적용됩니다.

### 잘 설계된 Spring Boot 프로젝트 구조

- **Domain-Driven Design** — 각 비즈니스 도메인(회원, 상품, 주문)별로 코드가 정리되어 비즈니스 로직이 명확합니다.
- **공통 예외 처리** — 모든 예외가 한 곳에서 관리되어 API 에러 응답이 일관되고 클라이언트 개발이 쉬워집니다.
...
**예시들:**
- `feat(member): 회원가입 이메일 인증 기능 추가`
- `fix(order): 주문 취소 시 상품 재고 복구 버그 해결`
- `refactor(product): ProductRepository 쿼리 최적화`
- `test(payment): 결제 실패 시나리오 테스트 추가`
